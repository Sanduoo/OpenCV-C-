...
//Mat(int rows,int cols,int type)高宽
//Mat(Size(int cols,int rows),int type)宽高
/*
其中：
type可分为：整型int 8S/16S/32S
			浮点型float 32F
			双精度浮点型double 64F
			无符号整型uchar 8U/16U
			(数字代表Mat中每个数值所占的bit数，32F为占四字节的float类型)
例如：CV_16S(1)表示：占2字节的int型单通道二维矩阵
	  CV_32F(3)表示：占4字节的float型三（多）通道三维矩阵
*/
